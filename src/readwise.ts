...
export default class ReadwisePlugin extends Plugin {
  ...
  async requestArchive(
    ...
    const parentDeleted = !(await this.app.vault.adapter.exists(
      this.settings.readwiseDir,
    ));
    ...
    try {
      ...
      console.log(
        'Readwise Official plugin: fetch failed in requestArchive: ',
        e,
      );
    }
    if (response && response.ok) {
      ...
      console.log(
        'Readwise Official plugin: bad response in requestArchive: ',
        response,
      );
      this.handleSyncError(
        buttonContext,
  ...
  async downloadArchive(
    ...
    try {
      ...
      console.log(
        'Readwise Official plugin: fetch failed in downloadArchive: ',
        e,
      );
    }
    if (response && response.ok) {
      ...
      console.log(
        'Readwise Official plugin: bad response in downloadArchive: ',
        response,
      );
      this.handleSyncError(
        buttonContext,
    ...
    if (entries.length) {
      for (const entry of entries) {
        ...
        const processedFileName = normalizePath(
          entry.filename.replace(/^Readwise/, this.settings.readwiseDir),
        );
        try {
          ...
          console.log(
            `Readwise Official plugin: error writing ${processedFileName}:`,
            e,
          );
          this.notice(
            `Readwise: error while writing ${processedFileName}: ${e}`,
            true,
            4,
            true,
          );
    ...
    if (this.app.isMobile) {
      this.notice(
        "If you don't see all of your readwise files reload obsidian app",
        true,
      );
    }
  }
  ...
  async acknowledgeSyncCompleted(buttonContext: ButtonComponent) {
    ...
    try {
      ...
      console.log(
        'Readwise Official plugin: fetch failed to acknowledged sync: ',
        e,
      );
    }
    if (response && response.ok) {
      ...
      console.log(
        'Readwise Official plugin: bad response in acknowledge sync: ',
        response,
      );
      this.handleSyncError(
        buttonContext,
  ...
  async configureSchedule() {
    ...
    console.log(
      'Readwise Official plugin: setting interval to ',
      milliseconds,
      'milliseconds',
    );
    ...
    this.scheduleInterval = window.setInterval(
      () => this.requestArchive(null, null, true),
      milliseconds,
    );
    ...
    try {
      fetch(`${baseURL}/api/refresh_book_export`, {
        ...
        if (response && response.ok) {
          ...
          this.settings.booksToRefresh = booksToRefresh.filter(
            (n) => !bookIds.includes(n),
          );
          this.saveSettings();
          return;
        } else {
      ...
      console.log(
        'Readwise Official plugin: fetch failed in refreshBookExport: ',
        e,
      );
    }
  }
  ...
  reimportFile(vault: Vault, fileName: string) {
    ...
    try {
      ...
      console.log(
        'Readwise Official plugin: fetch failed in Reimport current file: ',
        e,
      );
    }
  }
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      editorCheckCallback: (
        ...
        if (this.settings.reimportShowConfirmation) {
          ...
          const showConfContainer = modal.contentEl.createEl('div', {
            cls: 'rw-modal-confirmation',
          });
          showConfContainer.createEl('label', {
            attr: { for: 'rw-ask-nl' },
            text: "Don't ask me in the future",
          });
          const showConf = showConfContainer.createEl('input', {
            type: 'checkbox',
            attr: { name: 'rw-ask-nl' },
          });
    ...
    this.registerMarkdownPostProcessor((el, ctx) => {
      ...
      try {
        // @ts-ignore
        matches = [...ctx.getSectionInfo(el).text.matchAll(/__(.+)__/g)].map(
          (a) => a[1],
        );
      } catch (TypeError) {
      ...
      const hypers = el
        .findAll('strong')
        .filter((e) => matches.contains(e.textContent));
...
class ReadwiseSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    containerEl
      ...
      .createEl('a', { text: 'Readwise', href: 'https://readwise.io' });
    ...
    if (this.plugin.settings.token) {
      new Setting(containerEl)
        ...
        .setDesc(
          'On first sync, the Readwise plugin will create a new folder containing all your highlights',
        )
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          'You can customize which items export to Obsidian and how they appear from the Readwise website',
        )
        .addButton((button) => {
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          'By default, the plugin will save all your highlights into a folder named Readwise',
        )
        ...
        .addText((text) =>
          text
            ...
            .onChange(async (value) => {
              this.plugin.settings.readwiseDir = normalizePath(
                value || 'Readwise',
              );
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          'If not set to Manual, Readwise will automatically resync with Obsidian when the app is open at the specified interval',
        )
      ...
      new Setting(containerEl)
        ...
        .setDesc(
          'If enabled, Readwise will automatically resync with Obsidian each time you open the app',
        )
        .addToggle((toggle) => {
      ...
      if (this.plugin.settings.lastSyncFailed) {
        this.plugin.showInfoStatus(
          containerEl.find('.rw-setting-sync .rw-info-container').parentElement,
          'Last sync failed',
          'rw-error',
        );
      }
    ...
    if (!this.plugin.settings.token) {
      new Setting(containerEl)
        ...
        .setDesc(
          'The Readwise plugin enables automatic syncing of all your highlights from Kindle, Instapaper, Pocket, and more. Note: Requires Readwise account.',
        )
        .addButton((button) => {
          button
      ...
      let el = containerEl.createEl('div', { cls: 'rw-info-container' });
      containerEl
        .find('.rw-setting-connect > .setting-item-control ')
        .prepend(el);
    }
    const help = containerEl.createEl('p');
    help.innerHTML =
      "Question? Please see our <a href='https://help.readwise.io/article/125-how-does-the-readwise-to-obsidian-export-integration-work'>Documentation</a> or email us at <a href='mailto:hello@readwise.io'>hello@readwise.io</a> 🙂";
  }
}
